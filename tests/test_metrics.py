...
from pymaptools.testing import assert_true, assert_equal, assert_greater
...
class TestMetrics(unittest.TestCase):
    ...
    def test_perfect(self):
        ...
        self.assertAlmostEqual(cm.assignment_score(), 1.0, 4)
        self.assertAlmostEqual(cm.vi_similarity(), 1.0, 4)
        self.assertAlmostEqual(cm.split_join_similarity(), 1.0, 4)
        self.assertAlmostEqual(cm.talburt_wang_index(), 1.0, 4)
    ...
    def test_diseq(self):
        ...
        self.assertAlmostEqual(cf.diseq_coeff(), 0.0699, 4)
        cf = ConfusionMatrix2(rows=[[331, 331], [6.6, 331]])
        self.assertAlmostEqual(cf.cole_coeff(), 0.942, 3)
    ...
    def test_adjusted_mutual_info_score(self):
        ...
        self.assertAlmostEqual(mi_1, 0.41022, 5)
        mi_2 = mutual_info_score(labels_b, labels_a)
        self.assertAlmostEqual(mi_2, 0.41022, 5)
        ...
        self.assertAlmostEqual(ami, 0.37, 2)  # not accurate to more than 2 places


    def test_jaccard_nan(self):
    ...
    def test_entropy_of_counts_zero(self):
        """Returns zero for empty set
        """
        val = fentropy([])
        self.assertAlmostEqual(val, 0.0000, 4)
    ...
    def test_non_consecutive_labels_std(self):
    ...
    def test_non_consecutive_labels_ari(self):
        """regression tests for labels with gaps
        """
        ari_1 = adjusted_rand_score([0, 0, 0, 1, 1, 1], [0, 1, 0, 1, 2, 2])
        ari_2 = adjusted_rand_score([0, 0, 0, 1, 1, 1], [0, 4, 0, 4, 2, 2])
        self.assertAlmostEqual(ari_1, 0.24, 2)
        self.assertAlmostEqual(ari_2, 0.24, 2)
    ...
    def test_split_join(self):
        ...
        self.assertAlmostEqual(cm.vi_distance(normalize=False), 0.594, 3)
        ...
        self.assertAlmostEqual(cm.vi_distance(normalize=False), 0.520, 3)
        assert_equal(cm.split_join_distance(normalize=False), 4)


    def test_bc_metrics(self):
    ...
    def test_IR_example(self):
        ...
        rd = cm.row_diag()
        cd = cm.col_diag()
        self.assertAlmostEqual(rd.assignment_score(model='m3'), 1.0, 6)
        self.assertAlmostEqual(cd.assignment_score(model='m3'), 1.0, 6)
        self.assertAlmostEqual(cd.assignment_score(model='m3', discrete=True),   1.0, 6)
        self.assertAlmostEqual(rd.assignment_score(model='m3'), 1.0, 6)
        self.assertAlmostEqual(rd.assignment_score(model='m3', discrete=True),   1.0, 6)

        # test that no redraws happen by default
        self.assertAlmostEqual(cm.assignment_score(model='m3'),
                            cm.assignment_score(model='m3'), 6)

        ex = cm.expected(discrete=False)
        self.assertAlmostEqual(ex.assignment_score(model='m3'), 0.0, 6)
        ...
        self.assertAlmostEqual(h, 0.371468, 6)
        self.assertAlmostEqual(c, 0.357908, 6)
        self.assertAlmostEqual(v, 0.364562, 6)

        self.assertAlmostEqual(cm.vi_similarity(model=None),    0.517754, 6)
        self.assertAlmostEqual(cm.vi_similarity(model='m1'),    0.378167, 6)
        self.assertAlmostEqual(cm.vi_similarity(model='m2r'),   0.365605, 6)
        self.assertAlmostEqual(cm.vi_similarity(model='m2c'),   0.377165, 6)
        self.assertAlmostEqual(cm.vi_similarity(model='m3'),    0.364562, 6)

        self.assertAlmostEqual(cm.mirkin_match_coeff(),         0.695502, 6)
        self.assertAlmostEqual(cm.rand_index(),                 0.676471, 6)
        self.assertAlmostEqual(cm.fowlkes_mallows(),            0.476731, 6)
        self.assertAlmostEqual(cm.assignment_score(model=None), 0.705882, 6)
        self.assertAlmostEqual(cm.assignment_score(model='m3'), 0.554974, 6)

        self.assertAlmostEqual(cm.chisq_score(),          11.900000, 6)
        self.assertAlmostEqual(cm.g_score(),              13.325845, 6)

        # test metrics that are based on pairwise co-association matrix
        conf = cm.pairwise

        self.assertAlmostEqual(conf.chisq_score(),         8.063241, 6)
        self.assertAlmostEqual(conf.g_score(),             7.804221, 6)

        self.assertAlmostEqual(conf.jaccard_coeff(),       0.312500, 6)
        self.assertAlmostEqual(conf.ochiai_coeff(),        0.476731, 6)
        self.assertAlmostEqual(conf.dice_coeff(),          0.476190, 6)
        self.assertAlmostEqual(conf.sokal_sneath_coeff(),  0.185185, 6)

        self.assertAlmostEqual(conf.kappa(),               0.242915, 6)
        self.assertAlmostEqual(conf.accuracy(),            0.676471, 6)
        self.assertAlmostEqual(conf.precision(),           0.500000, 6)
        self.assertAlmostEqual(conf.recall(),              0.454545, 6)

        exp_tw = _talburt_wang_index(ltrue, lpred)
        act_tw = cm.talburt_wang_index()
        self.assertAlmostEqual(exp_tw, act_tw, 6)
    ...
    def test_twoway_confusion_1(self):
        ...
        self.assertAlmostEqual(cm.g_score(),       126.1, 1)
        self.assertAlmostEqual(cm.chisq_score(),   397.9, 1)

        mic0, mic1, mic2 = cm.mic_scores()
        self.assertAlmostEqual(mic2,       0.429, 3)
        self.assertAlmostEqual(mic1,       0.497, 3)
        self.assertAlmostEqual(mic0,       0.382, 3)

        self.assertAlmostEqual(cm.matthews_corr(), 0.377, 3)
        self.assertAlmostEqual(cm.informedness(),  0.523, 3)
        self.assertAlmostEqual(cm.markedness(),    0.271, 3)

        kappa0, kappa1, kappa2 = cm.kappas()
        self.assertAlmostEqual(kappa0,        0.267, 3)
        self.assertAlmostEqual(kappa1,        0.532, 3)
        self.assertAlmostEqual(kappa2,        0.355, 3)


    def test_twoway_confusion_2(self):
        """Finley's tornado data (listed in Goodman and Kruskal)
        """
        cm = ConfusionMatrix2.from_ccw(11, 14, 906, 3)

        self.assertAlmostEqual(cm.g_score(),       70.83, 2)
        self.assertAlmostEqual(cm.chisq_score(),   314.3, 1)

        mic0, mic1, mic2 = cm.mic_scores()
        self.assertAlmostEqual(mic0,      0.555, 3)
        self.assertAlmostEqual(mic1,      0.698, 3)
        self.assertAlmostEqual(mic2,      0.614, 3)

        self.assertAlmostEqual(cm.matthews_corr(), 0.580, 3)
        self.assertAlmostEqual(cm.informedness(),  0.770, 3)
        self.assertAlmostEqual(cm.markedness(),    0.437, 3)

        kappa0, kappa1, kappa2 = cm.kappas()
        self.assertAlmostEqual(kappa0,        0.431, 3)
        self.assertAlmostEqual(kappa1,        0.780, 3)
        self.assertAlmostEqual(kappa2,        0.556, 3)
    ...
    def test_negative_correlation(self):
        ...
        self.assertAlmostEqual(cm.g_score(),        384.52, 2)
        self.assertAlmostEqual(cm.chisq_score(),    355.70, 2)

        mic0, mic1, mic2 = cm.mic_scores()
        self.assertAlmostEqual(mic0,      -0.8496, 4)
        self.assertAlmostEqual(mic1,      -0.8524, 4)
        self.assertAlmostEqual(mic2,      -0.8510, 4)

        self.assertAlmostEqual(cm.matthews_corr(), -0.9012, 4)
        self.assertAlmostEqual(cm.informedness(),  -0.9052, 4)
        self.assertAlmostEqual(cm.markedness(),    -0.8971, 4)
        self.assertAlmostEqual(cm.kappa(),         -0.6407, 4)
        inform, marked = cm.informedness(), cm.markedness()
        expected_matt = geometric_mean(inform, marked)
        self.assertAlmostEqual(expected_matt, cm.matthews_corr(), 6)


    def test_twoway_confusion_phi(self):
        cm = ConfusionMatrix2.from_ccw(116, 21, 18, 21)
        self.assertAlmostEqual(cm.matthews_corr(), 0.31, 2)
        self.assertAlmostEqual(cm.yule_q(), 0.6512, 4)
        self.assertAlmostEqual(cm.DOR(),    4.7347, 4)

        cm = ConfusionMatrix2.from_ccw(35, 60, 41, 9)
        self.assertAlmostEqual(cm.chisq_score(), 5.50, 2)


    def test_kappa_precalculated(self):
        # from literature
        self.assertAlmostEqual(cohen_kappa(22, 4, 11, 2),
                            0.67, 2)
        self.assertAlmostEqual(product_moment(22, 4, 11, 2),
                            0.67, 2)
        self.assertAlmostEqual(cohen_kappa(147, 10, 62, 3),
                            0.86, 2)
        self.assertAlmostEqual(product_moment(147, 10, 62, 3),
                            0.87, 2)
        # numeric stability cases
        self.assertAlmostEqual(cohen_kappa(69, 1, 3, 11),
                            0.280000, 6)
        self.assertAlmostEqual(product_moment(69, 1, 3, 11),
                            0.350000, 6)
        self.assertAlmostEqual(cohen_kappa(1, 2, 96, 5),
                            0.191111, 6)
        self.assertAlmostEqual(product_moment(1, 2, 96, 5),
                            0.203746, 6)
