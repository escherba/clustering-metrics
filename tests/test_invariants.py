...
from pymaptools.testing import assert_true, assert_equal
...
class TestInvariants(unittest.TestCase):
    ...
    def test_m1(self):
        ...
        self.assertAlmostEqual(0.0, t2.vi_similarity_m1())
        self.assertAlmostEqual(0.0, t8.vi_similarity_m1())

        self.assertAlmostEqual(0.0, t2.split_join_similarity_m1())
        self.assertAlmostEqual(0.0, t8.split_join_similarity_m1())

        self.assertAlmostEqual(0.0, t2.assignment_score_m1())
        self.assertAlmostEqual(0.0, t8.assignment_score_m1())


    def test_RxC_general(self):
        ...
        for _ in range(100):
            ...
            for model in ['m1', 'm2r', 'm2c', 'm3']:
                ...
                self.assertAlmostEqual(
                    cm.grand_total,
                    sum(cm.expected(model=model).values()))
    ...
    def test_2x2_invariants(self):
        ...
        for _ in range(100):
            ...
            self._check_with_nans(v, geometric_mean(h, c), ensure_nans=False)

            # informedness
            actual_info = cm.informedness()
            expected_info_1 = cm.TPR() + cm.TNR() - 1.0
            expected_info_2 = cm.TPR() - cm.FPR()
            self._check_with_nans(actual_info, expected_info_1, 4, ensure_nans=False)
            self._check_with_nans(actual_info, expected_info_2, 4, ensure_nans=False)

            # markedness
            actual_mark = cm.markedness()
            expected_mark_1 = cm.PPV() + cm.NPV() - 1.0
            expected_mark_2 = cm.PPV() - cm.FOR()
            self._check_with_nans(actual_mark, expected_mark_1, 4, ensure_nans=False)
            self._check_with_nans(actual_mark, expected_mark_2, 4, ensure_nans=False)

            # matthews corr coeff
            # actual_mcc = cm.matthews_corr()
            # expected_mcc = geometric_mean(actual_info, actual_mark)
            # self._check_with_nans(actual_mcc, expected_mcc, 4, ensure_nans=False)
            ...
            self._check_with_nans(actual_kappa, expected_kappa_1, 4, ensure_nans=False)

            # kappa is the same as accuracy adjusted for chance
            expected_kappa_2 = harmonic_mean(*cm.adjust_to_null(cm.accuracy, model='m3'))
            self._check_with_nans(actual_kappa, expected_kappa_2, 4, ensure_nans=False)
            ...
            self._check_with_nans(actual_kappa, expected_kappa_3, 4, ensure_nans=False)
            ...
            self._check_with_nans(actual_odds_ratio, expected_odds_ratio, 4, ensure_nans=False)
            self._check_with_nans(actual_yule_q, expected_yule_q, 4, ensure_nans=False)

            # F-score and Dice
            expected_f = harmonic_mean(cm.precision(), cm.recall())
            actual_f = cm.fscore()
            self._check_with_nans(expected_f, actual_f, 6)
            self._check_with_nans(expected_f, cm.dice_coeff(), 6, ensure_nans=False)

            # association coefficients (1)
            dice = cm.dice_coeff()
            expected_jaccard = _div(dice, 2.0 - dice)
            actual_jaccard = cm.jaccard_coeff()
            self._check_with_nans(actual_jaccard, expected_jaccard, 6, ensure_nans=False)

            # association coefficients (2)
            jaccard = cm.jaccard_coeff()
            expected_ss2 = _div(jaccard, 2.0 - jaccard)
            actual_ss2 = cm.sokal_sneath_coeff()
            self._check_with_nans(actual_ss2, expected_ss2, 6, ensure_nans=False)

            # adjusted ochiai
            actual = cm.ochiai_coeff_adj()
            expected = harmonic_mean(*cm.adjust_to_null(cm.ochiai_coeff, model='m3'))
            self._check_with_nans(actual, expected, 6, ensure_nans=False)
